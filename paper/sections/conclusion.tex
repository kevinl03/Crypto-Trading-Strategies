...
As a classical bagging control within the same tree family---and a peer that is still uncommon in this cross-exchange microstructure setting---we also score a Random Forest on the same out-of-sample test set used for LightGBM. Predictions agree strongly ($\mathrm{corr}\approx 0.98$), confirming that ranking skill is not unique to gradient boosting, while LightGBM remains the reported model for higher R$^2$ and roughly twice as many trades at the absolute $|\hat{z}|\geq 0.9$ gate.
