...
We deliberately choose tree ensembles over the DNN/LSTM architectures common in prior spread-forecasting work: the feature matrix is a heterogeneous tabular panel (lags, microstructure, and native categoricals) for which tree models are a strong default inductive bias. As a classical bagging control on the same train/test feature matrix, we also fit a Random Forest regressor~\cite{breiman2001random} (regularized leaf size analogous to LightGBM's minimum child samples). LightGBM is the model used for validation-campaign evaluation; the forest peer is scored on the test set (Section~\ref{sec:results}).
