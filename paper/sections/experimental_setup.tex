...
For live validation, we run independent paper-trading campaigns that process incoming collector data in real time with no access to future snapshots. Unless noted, entry uses $|\hat{z}| \geq 0.5$ and $\text{max\_open} = 50$.
...
\subsubsection{Campaign C: Aug~4--7 (\textasciitilde72h)}

\begin{itemize}
    \item \textbf{Model}: same 68-feature, $H{=}1$, $w{=}300$ policy as Campaign~B.
    \item \textbf{Duration}: $\approx$72.9~hours of active trading (50,690 closed trades; 1.01M scored predictions).
    \item \textbf{Role}: longer live stress test of the $\tau{=}0.5$ book, and the evaluation surface for the post-hoc $\tau{=}0.9$ confidence gate (Section~\ref{sec:ablation}).
\end{itemize}

Campaigns~A/B are short weekday/weekend stress tests; Campaign~C extends the same production settings over three days. We present them as complementary live evidence rather than a controlled factorial experiment.
