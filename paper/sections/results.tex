...
Figure~\ref{fig:scatter} plots predicted versus realized z-scores for the Jul~31 filtered entries, illustrating the regression fit underlying R$^2 = 0.347$.

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{figures/fig2_pred_vs_realized_jul31.png}
\caption{Predicted vs realized z-scores on Jul~31 filtered entries ($|\hat{z}| \geq 0.5$). The model captures both direction and magnitude of short-horizon z-score dynamics.}
\label{fig:scatter}
\end{figure}
...
\caption{Cumulative z-unit proxy PnL over the Jul~31 session. The learned policy (solid) accumulates profit faster and more consistently than mechanical persistence (dashed). Mean-reversion (dotted) is uniformly negative.}
\label{fig:cumpnl}
\end{figure}
