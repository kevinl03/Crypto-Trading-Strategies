...
\subsection{Campaign~C (\textasciitilde72h) and Stricter Confidence Threshold}

Campaign~C extends the Campaign~B settings ($H{=}1$, $w{=}300$, $\tau{=}0.5$, $\text{max\_open}{=}50$) over $\approx$72.9~hours of active trading (50{,}690 closes; DirAcc 79.0\%; mean z-proxy $+0.837$; closed-only hourly Sharpe 4.38 with 74/74 green hours). This shows that the short Jul~31 window was not a one-off on the live $\tau{=}0.5$ book.

Applying a post-hoc $|\hat{z}|{\geq}0.9$ filter to the same closes---abstaining on weak forecasts rather than re-filling capacity under a new live $\tau$---cuts the book by $\approx$75\% while lifting DirAcc to 86.7\%, R$^2$ to 0.599, and per-trade Sharpe to 1.08 (Table~\ref{tab:tau_sensitivity}). The Jul~31 fine sweep identifies $\tau{=}0.9$ as the first point where per-trade Sharpe reaches ${\geq}1$, improving on the initial live threshold $\tau{=}0.5$ at the cost of coverage (Section~\ref{sec:ablation}).
