...
| `summary.json` | ✅ | Final trader stats at death (`updated_at` 09:36Z) |
| `dashboard.json` / `session_config.json` | ⚠️ | Post-death **restart** snapshot (`2026-08-07T16:40Z`); still carries pre-death counts |
...
- Last exit / death: `2026-08-07T09:36:28Z`
...
- `summary.json` `n_snaps=340` looks like a counter quirk (not used as a paper claim); use trade timestamps / collector snaps for duration.
...
| **B** Hourly equity + open MTM | Δ equity with `direction × z_t` on opens | **4.10** (mid-run report only) |
...
- Prefer headline **A = 4.38** (full book, closed-only) plus mid **B ≈ 4.10** if discussing open MTM; note full-book B was not re-saved to JSON.
...
5. **Limitations / lit framing:** z-proxy not dollars; no annualization; no “beats Tadi 7.94”; orphan inventory at kill; full-book B/MTM not re-saved.
...
- **Cleanup:** do not cite `dashboard.json` as death snapshot (it is a later restart).
