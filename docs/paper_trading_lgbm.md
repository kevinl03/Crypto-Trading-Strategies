...
4. **Trade** — open a paper bet when `|pred| >= entry_tau`, one per `(coin, pair)`, settled after
   `HORIZON=2` snapshots against the realized z-score.
...
Outputs per session: `config.json`, `signals.jsonl` (last 50k), `trades.jsonl`, `summary.json`.
...
- Collector cadence with `--slow-every 1` is ~110s per snapshot, not the nominal 60s. Size warmup expectations
  accordingly: 30 snapshots of z-score warmup take roughly an hour.
